  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-[#53544f] dark:bg-[#53544f]",
        className,
      )}
    >
      {children}
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium text-purple-500 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2",
        className,
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-white hover:text-[#beb3fe] dark:text-neutral-300"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "fixed top-0 z-50 mx-auto flex  w-[100vw] flex-col items-center justify-between bg-transparent px-0 py-2 lg:hidden",
...
export const MobileNavHeader = ({
  ...
  return (
    <div
      className={cn(
        "flex flex-row items-center w-[100vw] justify-between",
        className,
      )}
    >
      {children}
...
export const NavbarLogo = () => {
  return (
    ...
    >
      <img
        src="https://t4.ftcdn.net/jpg/03/62/59/85/360_F_362598596_Qvz2iQjTMNoMij8DvRfCrpcR1Y2lwccD.jpg"
        alt="logo"
        width={30}
        height={30}
        className="rounded-lg"
      />
      <span className="font-medium text-white dark:text-white">Hussain</span>
    </a>
  );
};
